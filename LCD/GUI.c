...
/**
 * @Function: GUI_DrawFont104(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s,uint8_t mode)
 * @Description: 检索104x56的数字
 */
void GUI_DrawFont104(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s,uint8_t mode)
{
	uint16_t i,j;
	uint16_t k;
	uint16_t HZnum;
	uint16_t x0=x;
	HZnum=sizeof(ud104)/sizeof(BIZ_UD104);	//自动统计汉字数目
	for (k=0;k<HZnum;k++) 
			{
			  if ((ud104[k].Index[0]==*(s))&&(ud104[k].Index[1]==*(s+1)))
			  { 	LCD_SetWindows(x,y,x+56-1,y+104-1);
				    for(i=0;i<728;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(ud104[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit(fc);
								else Lcd_WriteData_16Bit(bc);
							}
							else
							{
								POINT_COLOR=fc;
								if(ud104[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
								x++;
								if((x-x0)==56)
								{
									x=x0;
...
{					
    ...
    { 
        ...
        {   
			...
			GUI_DrawFont104(x,y,fc,bc,str,mode);	
